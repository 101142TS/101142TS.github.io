...
下面主要是写论文，对于我的方法而言，首先要获得一个初始的pDexFile, 有了pDexFile以后，各种数据收集和强制反射调用方法的故事才能继续往下说。

[获得pDexFile的方法](https://www.jianshu.com/p/ab9c3984d995)

dalvik虚拟机加载dex文件时，会使用Dalvik_dalvik_system_DexFile_openDexFileNative函数，该函数尝试把参数的文件当成后缀为.dex的文件打开，如果打开失败的话，则把它当做一个包含有classes.dex文件的Zip文件进行打开。

打开以后，存储到pDexOrJar中，并用addToDexFileTable最终添加到gDvm中的userDexFile结构的hash表中。

gDvm是一个DvmGlobals结构的全局变量。在DvmGlobals结构中有一个成员HashTable* userDexFiles; HashTable中有一个HashEntry* pEntries指向一个HashEntry的数组,HashEntry结构体中的data变量存放的就是pDexOrJar。

[获得pDexFile的classloader的方法](https://www.jianshu.com/p/23031578e654)

gDvm.loadedClasses存储着所有被加载的类，加载的类通过dvmAddClassToHash被添加到gDvm.loadedClasses中，添加类的方法和添加pDexFile的方法一样，都是用dvmHashTableLookup添加的